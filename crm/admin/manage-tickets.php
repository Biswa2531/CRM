...
// Handle secure ticket delete
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_ticket'])) {
  if (empty($_POST['csrf_token']) || !hash_equals($_SESSION['csrf_token'], $_POST['csrf_token'])) {
    die('Invalid CSRF token');
  }
  $delid = intval($_POST['id']);
  mysqli_query($con, "DELETE FROM ticket WHERE id=" . $delid);
...
<div class="page-container row"> 
 ...
 <input name="frm_id" type="hidden" id="frm_id" value="<?php echo $row['id'];?>" /></p>
                      ...
                      <form method="post" style="display:inline;margin-left:8px;" onsubmit="return confirm('Delete this ticket?');">
                        <input type="hidden" name="id" value="<?php echo $row['id'];?>" />
                        <input type="hidden" name="csrf_token" value="<?php echo $_SESSION['csrf_token'];?>" />
                        <button type="submit" name="delete_ticket" class="btn btn-danger btn-xs">Delete Ticket</button>
                      </form>
                    </div>
                    <div class="clearfix"></div>
                  </div>
